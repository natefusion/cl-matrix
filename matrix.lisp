...
(defun multiple (x y)
  (when (and (productp x) (productp y))
    (let* ((a (flatten x #'productp))
           (b (flatten y #'productp))
           (xor (set-exclusive-or a b))
           (intersection (intersection a b))
           (xor-var (remove-if #'numberp xor))
           (xor-num (remove-if-not #'numberp xor)))
      (when (not xor-var)
        (make-product (if (= 1 (length xor-num))
                          (1+ (first xor-num))
                          (reduce #'+ xor-num))
                      (if (= 1 (length intersection))
                          (first intersection)
                          (apply #'make-product (remove-if #'numberp (intersection a b))))
)))))
...
(defun make-sum (a b)
  (cond ((eql 0 a) b)
        ...
        ((and (numberp a) (numberp b)) (+ a b))
        ((and (negationp b)
              (exp-equal a (second b)))
         0)
        ((exp-equal a b) (make-product 2 a))
        (t (let ((multiple (multiple a b)))
             (if multiple multiple (list '+ a b))))))
